...
//----------------------------------------------------------------------------//
MyTrackFastSimEval::MyTrackFastSimEval(const string &name, const string &filename, const string &trackmapname)
  : SubsysReco(name)
...
//----------------------------------------------------------------------------//
int MyTrackFastSimEval::Init(PHCompositeNode *topNode)
{
  ...
  _eval_tree_tracks->Branch("nmeasurement", &nmeasurement, "nmeasurement/F");
...
//----------------------------------------------------------------------------//
int MyTrackFastSimEval::process_event(PHCompositeNode *topNode)
{
  _event++;
...
//----------------------------------------------------------------------------//
int MyTrackFastSimEval::End(PHCompositeNode *topNode)
{
...
//----------------------------------------------------------------------------//
void MyTrackFastSimEval::fill_track_tree(PHCompositeNode *topNode)
{
       ...
       truth_itr != range.second; ++truth_itr)
  {
    ...
    if (track)
    {
      ...
      ndf = track->get_ndf();
      nmeasurement = track->get_num_measurements();
...
//--   Fill the trees with truth, track fit, and cluster information
//----------------------------------------------------------------------------//
void MyTrackFastSimEval::fill_vertex_tree(PHCompositeNode *topNode)
{
  if (!_truth_container)
  {
    LogError("_truth_container not found!");
...
//----------------------------------------------------------------------------//
void MyTrackFastSimEval::reset_variables()
{
  event = -9999;
...
//----------------------------------------------------------------------------//
int MyTrackFastSimEval::GetNodes(PHCompositeNode *topNode)
{
  //DST objects
  //Truth container
...
void MyTrackFastSimEval::AddProjection(const string &name)
{
