...
int MyJetAnalysis_AllSi::Init(PHCompositeNode* topNode)
{
  ...
  //Trees
  m_T = new TTree("T", "MyJetAnalysis_AllSi Tree");
...
{
  if (Verbosity() >= MyJetAnalysis_AllSi::VERBOSITY_SOME)
    cout << "MyJetAnalysis_AllSi::process_event() entered" << endl;
  ...
  {
    cout
        << "MyJetAnalysis_AllSi::process_event - Error can not find DST JetMap node "
        << m_recoJetName << endl;
    exit(-1);
  }
