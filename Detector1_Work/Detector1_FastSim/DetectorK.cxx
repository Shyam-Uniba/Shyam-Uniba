...
void DetectorK::PlotLayout(Int_t plotDead) {
  ...
  for (Int_t i = fLayers.GetEntries()-1; i>=0; i--) {
    ...
    if (name.Contains("tpc") )  txt->SetTextSize(0.0);
...
void DetectorK::SolveViaBilloir(Int_t flagD0,Int_t print, Bool_t allPt, Double_t meanPt, char* detLayer) {
  ...
  Bool_t debug = false; // Change flag for more details
  ...
  // Total 9 Barrel layers+ 1 Beam Pipes = 10
  Int_t nLayer = fNumberOfActiveITSLayers;

  printf("N ITS Layers: %d\n",fNumberOfActiveITSLayers);

  // Min Track R: 132 coming from constructor; Last layer Radius = 77.46 cm Micromegas layer
  CylLayerK *last = (CylLayerK*) fLayers.At((fLayers.GetEntries()-1));

// Track Ptmin which will pass through all the layers; kPtMinFix = 0.05; kPtMaxFix = 31.5 from initializations
  Double_t bigRad = last->radius/2 ;	// min. pt which the algorithm below could handle 0.35357 GeV/c 
  ...
  double dlpt = log(ptmax/ptmin)/nPt; // log(31.5/0.35357)/50=4.48966/50 log scale
  
  for ( Int_t massloop = mStart ; massloop < 3 ; massloop++ )  { 
    
    // PseudoRapidity OK, used as an angle
    cout<<"Pseudo-Rapidity; "<<fAvgRapidity<<endl;
    lambda = TMath::Pi()/2.0 - 2.0*TMath::ATan(TMath::Exp(-1*fAvgRapidity)); // Average eta = 0.45 constructor
    ...
    for ( Int_t i = 0 ; i < nPt ; i++ ) { // pt loop
      ...
      fTransMomenta[i] =  ptmin*TMath::Exp(dlpt*i); // allPt flag = 1
      ...
      //
      // find max layer this track can reach Radius in cm so multiplied by 100.
    ...
    //cout<<"Layer Entries: "<<j<<" l->radius "<<l->radius<<" < 2*(rmx-5.): "<<2*(rmx-5.)<<endl;
	if (!(l->isDead) && (l->radius < 2*rmx)) {lastActiveLayer = j; last = l; break;} // 2*(rmx-5.) replaced by 2*rmx shyam
      }
      if (lastActiveLayer<0) {
	printf("No active layer with radius < %f is found, pt = %f\n",rmx, pt); 
	return;
      }
      //cout<<"Last Active Layer;"<<lastActiveLayer<<" Radius; "<<last->radius<<endl;
      //      printf("PT=%f 2Rpt=%f Rlr=%f\n",pt,2*rmx,last->radius);
      if (debug){
      cout<<"\033[1;31m Initial Track Parameters from Verex to  some larger than LastLayer\033[0m\n";
      probTr.Print();
      }
      //
      ...
      if (debug){
      cout<<"\033[1;31m Final Track Parameters from Verex to some larger than LastLayer\033[0m\n";
      probTr.Print();
      }  
      ...
      if (debug){
      cout<<"\033[1;31m Resetting Large Errors in Covariance Matrix\033[0m\n";
      probTr.Print();  
      }
	...
	if (layer->radius>fMaxSeedRadius) continue; // no seeding beyond this radius constructor (Max Radius; 50000)
  ...
  //probTr.Print();
      if (debug){
      cout<<"======================================================================"<<endl;
      cout<<"\033[1;31m Propagating Parameters to Layer\033[0m:  "<<name<<endl;
      probTr.Print();
      }  
	...
	//	*/
	// save resolutions at this layer: commented on July 2022: Shyam crashing the code after adding TPC
	...
	// End save
	//
	if (isVertex) continue; // Skip the vertex
	//
	...
	//
    if (debug) printf("Update the track by measurement {%.3f,%3f} err {%.3e %.3e %.3e}\n",
	...
	if (!probTr.Update(meas,measErr2)) {
	  ...
	  exit(1);
	      ...
	      if (debug){cout<<"\033[1;31m AfterUpdate Measurment \033[0m\n"; probTr.Print();}
	...
	if (!probTr.CorrectForMeanMaterial(layer->radL, 0, mass[massloop] , kTRUE)) {
      ...
      //cout<<"Layer; "<<j<<endl;
	       if (debug){cout<<"\033[1;31m After MS correction \033[0m\n"; probTr.Print();} // Propagates the parameter till layer 1 beampipe x = radius = 3.1 cm
	//
      } // layer loop

     
           if (debug){cout<<"\033[1;31m Final Extrapolation to Vertex\033[0m\n"; probTr.Print();} // Final extraploation toward vertex x = radius = 0;
      // exit(0); // Uncomment if we want to see only single propogation
  //
      ...
      cout<<"Pt: "<<pt<<" Pt Resol: "<<fMomentumRes[i]<<" RPhi Resol: "<<fResolutionRPhi[i]<<" Z Resol: "<<fResolutionZ[i]<<endl;
          // Calculation of Momentum Resolution
      fTotalMomenta[i] = probTr.P(); // Momentum
      Double_t TanLambda = probTr.GetTgl();
      Double_t One_Pt = TMath::Abs(probTr.GetSigned1Pt());
      Double_t Sigma_One_Pt = TMath::Sqrt(probTr.GetSigma1Pt2());
      ...
      Double_t corrfactor = 0.0; // Assuming 100% correlation (tune it from 0. to 1.)
      Double_t corr_term = 2.0*corrfactor*term1*term2;

      Double_t TotalPResol = TMath::Sqrt(term1*term1+term2*term2+corr_term);
      fTotalMomentumRes[i] = 100.*TotalPResol/fTotalMomenta[i];
    } // pt loop
...
TGraph * DetectorK::GetGraphTotalMomentumResolution(Int_t color, Int_t linewidth) {
  //
  // returns the momentum resolution 
  //
  
  TGraph *graph = new TGraph(kNptBins, fTotalMomenta, fTotalMomentumRes);
  graph->SetTitle("Momentum Resolution .vs. Pt" ) ;
  //  graph->GetXaxis()->SetRangeUser(0.,5.0) ;
...
void DetectorK::MakeStandardPlots(Bool_t add, Int_t color, Int_t linewidth,Bool_t onlyPionEff) {
  ...
  if (!add) {
    ...
    PtRes = GetGraphMomentumResolution(color,linewidth);
    PtRes->SetName(Form("grMomRes%d",1));
    PtRes->Draw("AL");
    
    c1->cd(3); gPad->SetGridx();   gPad->SetGridy(); 
    gPad->SetLogx(); 
    pointResXY = GetGraphPointingResolution(0,color,linewidth);
    pointResXY->SetName(Form("pointRRes%d",0));
    pointResXY->Draw("AL");
    //
    ...
    PResol = GetGraphTotalMomentumResolution(color,linewidth);
