...
CeeTpcElectronicsTask::CeeTpcElectronicsTask()
	...
	fmaxmctrackId(1000),fPedestal(0),fNoiseTimeBins(1), fSignalStart(0.), fSignalStop(0.),fMaxPadsinRow(240.)
 {
  ...
  fSignalfile = TFile::Open("../tpc/DigiTpc_new/signal.root");
  fNoisefile = TFile::Open("../tpc/DigiTpc_new/noise_hist.root");
...
CeeTpcElectronicsTask::Exec(Option_t* opt)
{
  ...
  int region = 0, timebin = 0, mctrackId = 0, id = 0, is = 0, sectorid = 0, rowid = 0;
  ...
  for (padIt = padmap.begin(); padIt!=padmap.end(); padIt++){
	 // if (is>0) continue;
	  ...
	  sectorid = (*pad_iter)->sectorId();
	  rowid = (*pad_iter)->rowId();
   ...
   float drift_timebin = (((*pad_iter)->t())/ftimestep);
	  ...
	  CeeTpcDigi* digi = new((*fdigiArray)[is]) CeeTpcDigi(padIt->first,is, sectorid,rowid,tbin,adc,mcId);
	  is++;
	  }
	  if (fdebug) std::cout<<"\n"<<std::endl;
  }
