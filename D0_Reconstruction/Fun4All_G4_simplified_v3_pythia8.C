 ...
 gSystem->Setenv("PYTHIA8DATA",Form("%s/xmldoc",getenv("PYTHIA8")));
  ...
  pythia8->print_config();
	...
	if(magnetic_field==1){          // uniform 1.5T
		...
		TString path_to_map = "/home/shyam/Singularity/Tracker_Performance/magfield/EIC_Magnetic_Field_Map_2021_05_07_radial_coords_[cm]_[T].120000.lines.Bmap";
		g4Reco->set_field_map(string(path_to_map), PHFieldConfig::kFieldBeast);
	}
	else if(magnetic_field==6){
		B_label = "_B_ATHENA_210528";
		TString path_to_map = "/home/shyam/Singularity/Tracker_Performance/magfield/EIC_v.0.1.0_Magnetic_Field_Map_2021_05_28_radial_coords_[cm]_[T].401301.line.Bmap";
		g4Reco->set_field_map(string(path_to_map), PHFieldConfig::kFieldBeast);
	}
	else if(magnetic_field==7){
		B_label = "_B_ATHENA_2109";
		TString path_to_map = "additional_B_fields/EIC_v.2.0.3_Magnetic_Field_Map_2021_09_28_radial_coords_cm_T.Bmap.txt";
		g4Reco->set_field_map(string(path_to_map), PHFieldConfig::kFieldBeast);
	}
	else if(magnetic_field==8){
		B_label = "_B_ATHENA_210507_shifted";
		TString path_to_map = "additional_B_fields/ATHENA0507_Shifted.Bmap";
                g4Reco->set_field_map(string(path_to_map), PHFieldConfig::kFieldBeast);
	}
	...
	double max_disk_R = 43.23;

	for(int i = 0 ; i < nDisks ; i++){
		si_r_max[i] = TMath::Min(max_disk_R,18.5*abs(si_z_pos[i])/si_z_pos[nDisks_per_side]);
