...
// GEM_support_Detector::ConstructMe() method
// but the convention is as mentioned cm and deg
//____________________________________________________________________________..
//
#include "GEM_support_Subsystem.h"

#include "GEM_support_Detector.h"
#include "GEM_support_SteppingAction.h"
...
GEM_support_Subsystem::GEM_support_Subsystem(const std::string &name)
...
//_______________________________________________________________________
int GEM_support_Subsystem::InitRunSubsystem(PHCompositeNode *topNode)
{
  ...
  // create detector
  m_Detector = new GEM_support_Detector(this, topNode, GetParams(), Name());
  m_Detector->OverlapCheck(CheckOverlap());
  // create stepping action if detector is active
  if (GetParams()->get_int_param("active"))
  {
    m_SteppingAction = new GEM_support_SteppingAction(m_Detector, GetParams());
  }
  return 0;
}
//_______________________________________________________________________
int GEM_support_Subsystem::process_event(PHCompositeNode *topNode)
{
...
//_______________________________________________________________________
void GEM_support_Subsystem::Print(const string &what) const
{
  if (m_Detector)
  {
...
//_______________________________________________________________________
PHG4Detector *GEM_support_Subsystem::GetDetector(void) const
{
  return m_Detector;
}

//_______________________________________________________________________
void GEM_support_Subsystem::SetDefaultParameters()
{
  // sizes are in cm
  // angles are in deg
