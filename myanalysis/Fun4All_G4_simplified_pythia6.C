...
void Fun4All_G4_simplified_pythia6(
...
//http://fismed.ciemat.es/GAMOS/GAMOS_doc/GAMOS.5.1.0/x11519.html
	...
	if(particle_gen==1){se->registerSubsystem(   gen); cout << "Using particle generator"             << endl;}
		...
		do_pythia6_jets = true;
		gSystem->Load("libpythia6eRHIC.so");

		PHPythia6 *pythia6 = new PHPythia6(); // see coresoftware/generators/PHPythia8 for example config
		pythia6->set_config_file("phpythia6.cfg"); // example configure files : https://github.com/sPHENIX-Collaboration/coresoftware/tree/master/generators/PHPythia8 
		pythia6->print_config();
	 pythia6->Verbosity(0);
 ...
 hits->AddNode("FGEM", 4);
	...
	if(do_pythia6_jets) Jet_Reco();
	if(do_pythia6_jets) Jet_Eval(string(outputFile) + "_g4jet_eval.root");
