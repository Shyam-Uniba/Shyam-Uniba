...
#include <g4lblvtx/MyTrackFastSimEval.h>
...
double EtaFromZR(double Z, double R)
{
	double Eta = -1*TMath::Log(TMath::Tan(0.5*TMath::ATan(R/Z)));
	return Eta;
}

void Fun4All_G4_Baseline2_D0(
...
//https://www.star.bnl.gov/public/comp/simu/newsite/gstar/Manual/particle_id.html
	...
	gen->set_name("D0");	// geantino, pi-, pi+, mu-, mu+, e-., e+, proton, ... (currently passed as an input)
	gen->set_vtx(0,0,0);			// Vertex generation range
	gen->set_mom_range(pmin,pmax);		// Momentum generation range in GeV/c
	//gen->AddParticle(421,gRandom->Uniform(1.0,1.0),gRandom->Uniform(1.0,1.0),gRandom->Uniform(1.0,1.0));
//	gen->set_t0(0.0004); // 4.0*e-13 sec
	gen->set_z_range(0.,0.);
	...
	gen_pT->set_eta_range(0., 0.);               // Detector coverage corresponds to |η|< 4
...
+Gap_betweenlayer/2, 80-(nCZlayer*thicknessMPGD+gap_betweenCZ)*2-Gap_betweenlayer, 80-nCZlayer*thicknessMPGD-gap_betweenCZ};                          
	...
	*/
	...
	//hardcode baseline 2
	double bmt_length_outer = 201.98;// (1-exp(-2*prapidity))/exp(-prapidity)*BMT_r[2] -2.5;
	double bmt_length_inner = 127.47;//(1-exp(-2*prapidity))/exp(-prapidity)*BMT_r[0] -2.5;
