void PlotFastSimOutput(float etamin, float etamax){
  ...
  TFile *f[nbins];
  TGraph *grPt[nbins], *grPRXY[nbins], *grPRZ[nbins];
    ...
    for (int i=0; i<nbins; i++){
        ...
        double x[NPt], AvgPtRes_Arr[NPt], AvgPR_XY_Arr[NPt], AvgPR_Z_Arr[NPt];
        TGraph *grPt_Avg, *grPRXY_Avg, *grPRZ_Avg;


        for (int ipoint =0; ipoint<NPt; ipoint++){ // X -axis

          double Pt, PtRes=0., PR_XY=0., PR_Z=0.;  
          double AvgPtRes =0., AvgPR_XY=0., AvgPR_Z=0.;	
