...
void pythia8_eventdisplay()
{
   TString dir = gSystem->UnixPathName(__FILE__);
   dir.ReplaceAll("pythia8_eventdisplay.C","");
   dir.ReplaceAll("/./","/");
...
// Load libraries
   ...
   gROOT->ProcessLine("#include \"pythia8_eventdisplay.C\"");
...
// Initialize
  ...
  TString rootfile="g4test.gdml"; // root geo or gdml will work Eic Tracker
  TEveManager::Create();
  ...
  TEveGeoTopNode *TNod = new TEveGeoTopNode(gGeoManager,gGeoManager->GetTopNode());
   ...
   gMultiView->ImportGeomRPhi(TNod);
   gMultiView->ImportGeomRhoZ(TNod);
