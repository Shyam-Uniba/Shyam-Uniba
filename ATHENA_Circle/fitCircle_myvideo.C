 ...
 void fitCircle_myvideo(Int_t n=6, Double_t pT = 1.0) { // pT = 1 GeV/c
    //generates n points around a circle and fit them
    TCanvas *c1 = new TCanvas("c1","c1",1200,1000);
    c1->SetGrid();
    TH1 *frame = c1->DrawFrame(-8,-8,8,8);
 ...
 //   result.Print(std::cout);
  ...
  Circle_Intersection c_2(rad,0., 0.);
  ...
  a = gRandom->Uniform(-1.0*Radius,Radius);
   ...
   TPaveText *pt = new TPaveText(0.12,0.9,0.2,0.99,"NDC");	
   pt->SetBorderSize(0);
   pt->SetFillStyle(0);
   pt->SetTextSize(0.035);
   pt->SetTextColor(kGreen);
   pt->AddText(Form("Track p_{T} = %1.4f (GeV/c)",pT));
   pt->Draw("same");

   pt = new TPaveText(0.4,0.9,0.6,0.99,"NDC");	
   pt->SetBorderSize(0);
   pt->SetFillStyle(0);
   pt->SetTextSize(0.035);
   pt->SetTextColor(kBlack);
   pt->AddText("B = 3 T (Z-direction)");
   pt->Draw("same");



   pt = new TPaveText(0.80,0.95,0.95,0.99,"NDC");	
   pt->SetBorderSize(0);
   pt->SetFillStyle(0);
   pt->SetTextSize(0.04);
   pt->SetTextColor(kBlue);
   pt->AddText("Silicon Layers");
   pt->Draw("same");
   pt = new TPaveText(0.80,0.90,0.95,0.96,"NDC");	
   pt->SetBorderSize(0);
   pt->SetFillStyle(0);
   pt->SetTextSize(0.04);
   pt->SetTextColor(kMagenta);
   pt->AddText("Hit Points");
   pt->Draw("same");
   ...
   pt->Draw("same");*/
   c1->SaveAs(Form("track_pt_%1.4f_GeVperc.png",pT));
}
